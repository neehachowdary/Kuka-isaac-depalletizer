...
def plan_and_extend(start, goal_pose_list, all_positions, label):
    ...
    print(f"  {label}: {len(pos)} waypoints, motion_time={result.motion_time.item():.3f}s")
...
# Via-point: a fixed safe-height checkpoint the arm must pass through
# between the pallet stack and the conveyor
via_point_x = 1.3
via_point_y = 0.0
via_point_z = 0.9
...
for box_num, box_idx in enumerate(pick_order):
    ...
    if not result.success.item():
        ...
        print(f"  approach_and_grasp: {len(pos)} waypoints, motion_time={result.motion_time.item():.3f}s")

    # Stage 1: lift to via-point (required checkpoint)
    current_state, seg = plan_and_extend(current_state, [via_point_x, via_point_y, via_point_z, 0.0, 1.0, 0.0, 0.0], all_waypoints, "lift_to_via_point")
    segment_info.append((box_idx, "lift_to_via_point", len(seg) if seg is not None else 0))

    # Stage 2: via-point to conveyor
    current_state, seg = plan_and_extend(current_state, [px, py, pz + approach_height, 0.0, 1.0, 0.0, 0.0], all_waypoints, "via_point_to_conveyor")
    segment_info.append((box_idx, "via_point_to_conveyor", len(seg) if seg is not None else 0))
    ...
    if not result.success.item():
        ...
        print(f"  descend_place: {len(pos)} waypoints, motion_time={result.motion_time.item():.3f}s")
