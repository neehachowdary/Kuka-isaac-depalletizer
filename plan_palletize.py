...
for box_num, box_idx in enumerate(pick_order):
    ...
    current_state, seg = plan_and_extend(current_state,
        [px, py, pz + approach_height, 0.0, 1.0, 0.0, 0.0], all_waypoints, "move_to_conveyor")
    segment_info.append((box_idx, "move_to_conveyor", len(seg) if seg is not None else 0))

    current_state, seg = plan_and_extend(current_state,
        [px, py, pz + place_clearance, 0.0, 1.0, 0.0, 0.0], all_waypoints, "descend_place")
    segment_info.append((box_idx, "descend_place", len(seg) if seg is not None else 0))

    current_state, seg = plan_and_extend(current_state,
        [px, py, pz + approach_height, 0.0, 1.0, 0.0, 0.0], all_waypoints, "retreat")
